...
public class NewSteeringBehavior : MonoBehaviour
{
    ...
    /******CONCEPTOS Y CÓDIGO EXPLICADOS O MENCIONADOS EN CLASE *******/

    // 5*5 + 2*2 = if(sqrt(29) > maxSpeed) 

    //Mathf.Max(0.0f, 5.0f); // devuelve 5
    //Mathf.Min(0.0f, 5.0f); // devuelve 0

    //Mathf.Clamp(iCounter, 0, 5.0f);
    //Mathf.Ceil
    //Mathf.Floor

    // Vector3 v3CurrentVelocity = currentVelocity;
    //transform.forward += v3CurrentVelocity * Time.deltaTime;
    ...
    // Notación húngara
    //int iCounter;  // prefijo i para enteros
    //float fValue; // prefijo f para float
    //bool bFlag; // prefijo b para booleanos
    //Vector2 vVector; // prefijo v para vector
    //List<Vector2> lMyList;
    ...
    //{
    //    iBestValue = iCounter;
    //    for (jCounter = 0; jCounter < 100; jCounter++)
    //    {
